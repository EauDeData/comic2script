...
class PredictionLoss(torch.nn.Module):
    ...
    def forward(self, masks, original):
        ...
        output = output * masks + self.epsilon
        center = F.adaptive_avg_pool2d(output, (1, 1)).view(original.shape[0], -1)
        res = output - center.view(-1, 21, 1, 1)
...
# Maybe the approach is just not right
# If the classifier does not find any relevant feature, it's shit
...
if __name__ == '__main__':
    ...
    l = PredictionLoss()
    bs = 5
    print(l(torch.ones(bs, 1, 64, 64, requires_grad = True), torch.ones(bs, 3, 64, 64)).shape)
